...
export default {
  createPost ({commit, state}, post) {
    const postId = firebase.database().ref('vue-forum/posts').push().key
    ...
    firebase.database().ref('vue-forum').update(updates)
      .then(() => {
  ...
  createThread ({state, commit, dispatch}, {text, title, forumId}) {
    return new Promise((resolve, reject) => {
      const threadId = firebase.database().ref('vue-forum/threads').push().key
      const postId = firebase.database().ref('vue-forum/posts').push().key
  ...
  createUser ({state, commit}, {id, email, name, username, avatar = null}) {
    return new Promise((resolve, reject) => {
      ...
      firebase.database().ref('vue-forum/users').child(id).set(user)
        .then(() => {
  ...
  signInWithGoogle ({dispatch}) {
    ...
    return firebase.auth().signInWithPopup(provider)
      .then(data => {
        ...
        firebase.database().ref('vue-forum/users').child(user.uid).once('value', snapshot => {
          if (!snapshot.exists()) {
  ...
  updatePost ({state, commit}, {id, text}) {
    return new Promise((resolve, reject) => {
      ...
      firebase.database().ref('vue-forum/posts').child(id).update(updates)
        .then(() => {
  ...
  fetchAuthUser ({dispatch, commit}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      firebase.database().ref('vue-forum/users').child(userId).once('value', snapshot => {
        if (snapshot.exists()) {
  ...
  fetchAllCategories ({state, commit}) {
    console.log('🔥', '🏷', 'all')
    return new Promise((resolve, reject) => {
      firebase.database().ref('vue-forum/categories').once('value', snapshot => {
  ...
  fetchItem ({state, commit}, {id, emoji, resource}) {
    ...
    return new Promise((resolve, reject) => {
      firebase.database().ref(`vue-forum/${resource}`).child(id).once('value', snapshot => {
